...
#cmt_local_tagfile_install_more_includes = $(SamplingSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(SamplingSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)SamplingSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)SamplingSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "SamplingSvc" = ""; then \
	    if test -d ../SamplingSvc; then \
	      echo "Installing files from more ../SamplingSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/SamplingSvc; then mkdir -p $(install_include_dir)/SamplingSvc; fi;\
	        $(install_command) ../SamplingSvc $(install_include_dir);\
	    else \
	      echo "No more SamplingSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/SamplingSvc"; \
	  $(uninstall_command) "$(install_include_dir)/SamplingSvc/*" ; \
	  if test -f $(install_include_dir)/SamplingSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/SamplingSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/SamplingSvc.cmtref ; \
	  fi; \
	fi
